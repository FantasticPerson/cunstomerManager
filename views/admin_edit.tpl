...
<body>
	<div class="contentBox">
		<form id="dataform" name="dataform" enctype="multipart/form-data" method="post" action="{{.baseurl}}/superadmin/saveAdmin.do">
            ...
            <input type="hidden" id="DepartIds" name="did" value="">
            <input type="hidden" id="oAdminPassword" name="oAdminPassword" value="">
		</form>
	</div>
	<div class="buttonBar">
...
$(function(){
	var data = parent.window.__cache__.data;
	  ...
	  console.log(target);
	  $("#adminRoleName").val(target["RealName"]);
	  $("#adminName").val(target["UserName"]);
	  $("#adminPass").val(target["UserPass"]);
	  $("#RoleId").val(target["Id"]);
});
</script>
</html>