...
<body>
	<div class="contentBox">
		<form id="dataform" name="dataform" enctype="multipart/form-data" method="post" action="">
			<input type="hidden" id="inputKeywordid" value="{{.data.Id}}">
			<fieldset>
				...
				<table width="99%" border="0" align="center" cellpadding="0"
					...
					<tr>
						...
						<td width="30%"><input type="text" class="textInput width_205" id="inputKeyword" value="{{.data.KeyWord}}"></td>
						<td></td>
					</tr>
					<tr>
						<td height="35" align="right">替换文字：</td>
						<td><input type="text" class="textInput width_205" id="inputReplace" value="{{.data.ReplaceWord}}">
						</td>
						<td></td>
					</tr>
				</table>
	...
	<div class="buttonBar">
		<input type="submit" onclick="javascript:parent.dialog.close();"
			...
			onclick="updateKeyword();" value="保存" class="confirmBtn" />
	</div>
</body>
...
function updateKeyword(){
	var inputName=$.trim($('#inputKeyword').val());
	 ...
	 var mid = $.trim($('#inputKeywordid').val());
     if(inputName==""){
        alert("请输入敏感词！");
        return;
     }
