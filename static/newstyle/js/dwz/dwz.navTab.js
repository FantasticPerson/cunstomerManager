...
//		this._contextmenu(this._getTabs());
	...
	_init: function(){
		...
		this._switchTab(this._currentIndex);
			//修复：页面缩小后，由于别的tab被隐藏，导致别的tab隐藏宽度重新计算，重新切换回来，宽度不正确
			$(window).resize();
	},
		...
		if ($panel){
			...
			if (url) {
				...
				if ($tab.hasClass("external")) {
					...
					$panel.ajaxUrl({
						type:method?method:"POST", url:url, data:op.data, callback:function(response){
