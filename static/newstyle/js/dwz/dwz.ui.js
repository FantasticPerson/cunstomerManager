...
function initLayout(){
	var iContentW = $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() + 10 : 34) - 5 -80;
	var iContentH = $(window).height() - $("#header").height() - 29;

	$("#container").width(iContentW);
	$("#container .tabsPageContent").height(iContentH).find("[layoutH]").layoutH();
	//$("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH - 1); //iContentH - 5
...
function initUI(_box){
	...
	$("div.pagination", $p).each(function(){
		...
		$this.pagination({
			...
			isOnclick_pagination:$this.attr("isOnclick_pagination"),
			isOnclick_return:$this.attr("isOnclick_return"),
			url:$this.attr("url"),
