...
(function($){
    $.extend($.fn, {
        jTree:function(options) {
            ...
            return this.each(function(){
                ...
                $(">li", $this).each(function(){
                    ...
                    var first = $li.prev()[0]?false:true; //第一个二级
        ...
        genTree:function(options) {
            var op = $.extend({icon:options.icon,ckbox:options.ckbox,exp:"", coll:"", showSub:false, level:0, options:null, isLast:false,genStyle:options.options.genStyle,subStyle:options.options.subStyle}, options);
            return this.each(function(){
                var node = $(this);
                var tree = $(">ul", node);
                var gtree= $(this).parent().hasClass("tree");
                var parent = node.parent().prev();
                var checked = 'unchecked';
                if(op.ckbox) {
                    if($(">.checked",parent).size() > 0) checked = 'checked';
                }
                if (tree.size()>0 || gtree) {
                    node.children(":first").wrap("<div class='"+op.genStyle+"'></div>");
                    ...
                    $(">div>div:first,>div>a", node).click(function(){
                      ...
                      //  }
                        tree.is(":hidden")?tree.slideDown("fast"):tree.slideUp("fast");//isA?"":tree.slideUp("fast")
                        return false;
                    });
                    addSpace(op.level, node);
                    if(op.showSub) tree.subTree(op, op.level + 1);
                } else {
                    node.children().wrap("<div class='"+op.subStyle+"'></div>");
