...
UI.TextField=function(A){
  ...
  this.dom=this.jsrvalue=Jsr.extend(jsr(A.id),JTag);
...
UI._textField={render:function(){
    ...
    this.dom.setWidth(jsrP(this.dom).clientWidth);
  },check:function(){
    if(this.vMethod){
...
UI.SpinnerField=function(B){
  ...
  var C=this,A;
  C.jsrvalue=Jsr.extend(jsr(B.id),JTag);
  C.jsrvalue.id=Jsr.getTagID();
  C.jsrvalue.name=B.name||B.id;
  C.jsrvalue.value=B.value||0;
  C.jsrvalue.readOnly=true;
  isFF&&(C.jsrvalue.size=200);
  C.dom=Jsr.extend(jsrB("div"),JTag);
  (A=jsrP(C.jsrvalue))&&A.insertBefore(C.dom,C.jsrvalue);
  C.min=B.minValue||0;
...
UI._spinnerField={render:function(){
    ...
    B.onblur=function(){
      if(!/^[-\+]?\d+jsr/.test(this.value)||this.value<H.min||this.value>H.max){
        this.value=D;
      }
      if(E){
        C.setClass("text-bg border-blur");
    ...
    A[0].setBgPos(C[0]);
    A[1].setBgPos(C[1]);
    this.jsrvalue.disabled=B;
    C=A=null;
  },reset:function(){
    this.jsrvalue.value=this.para.value||this.min;
...
UI.listModel={initList:function(C){
    var D=this,G=jsr(C.id),A=[],E=[],F=-1;
    D.items=[];
    D.texts=[];
    D.values=[];
    D.selectIndex=-1;
    ...
    if(!A.listType){
      ...
      D.onmousedown=function(){
        ...
        A.setValue({value:A.values[this.index]});
      };
    }
    A.listPanel.add(D);
    A.texts.push(C);
    ...
    this.items.clear();
    this.selectIndex=-1;
    !this.listType&&(this.text.value=this.jsrvalue.value="");
...
UI.ListView=function(A){
  ...
  this.jsrvalue=jsrB("input");
  this.jsrvalue.type="hidden";
  this.jsrvalue.name=A.name||A.id;
  this.listType=true;
  this.initList(A);
  this.dom=this.listPanel;
  this.dom.add(this.jsrvalue);
  A=null;
};
UI._listView={getValue:function(){
...
UI.comboModel={initCombo:function(C){
    var D=this,B=jsr(C.id),A=true;
    D.jsrvalue=jsrB("input");
    D.jsrvalue.type="hidden";
    D.jsrvalue.name=C.name||C.id;
    D.dom=Jsr.extend(jsrB("div"),JTag);
    jsrP(B)&&jsrP(B).insertBefore(D.dom,B);
    ...
    D.dom.add(D.jsrvalue);
    D.setWidth(C.width);
    D.panel=Jsr.extend(document.body.appendChild(jsrB("div")),JTag);
    ...
    return this.jsrvalue.value.trim();
  },setValue:function(A){
    var B=this;
    if(B.selectByIndex){
      ...
      B.text.value=B.jsrvalue.value=A;
    }
    B.onChangeCallBack&&B.onChangeCallBack();
    B=null;
  },setCheck:function(C,A,B,D){
...
UI._comboPanel={setPanel:function(B){
    var A=this;
...
UI.ComboBox=function(B){
  ...
  C.text=jsrB("input");
  C.initCombo(B);
  jsrP(A)&&jsrP(A).insertBefore(C.dom,A);
  C.initList(B);
...
UI.crModel={callBack:null,init:function(){
    var B=this,A;
    B.jsrvalue=Jsr.extend(jsr(B.para.id),JTag);
    B.jsrvalue.id=Jsr.getTagID();
    B.jsrvalue.name=B.para.name||B.para.id;
    B.jsrvalue.value=B.para.value||B.type;
    B.dom=Jsr.extend(jsrB("div"),JTag);
    (A=jsrP(B.jsrvalue))&&A.insertBefore(B.dom,B.jsrvalue);
    B.dom.setHTML("<div></div><div><nobr unselectable=on></nobr></div><div style='clear:both'></div>");
    B.dom.setStyle("padding:1px 0 1px 10px;");
    B.dom.add(B.jsrvalue).setDisplay("none");
    A=B.dom.childNodes;
    ...
    return this.jsrvalue.checked;
  },checked:function(A){
    if(this.jsrvalue.disabled){
      return ;
    }
    if((this.jsrvalue.checked=A)){
...
UI.CheckBox=function(A){
  ...
  this.onChangeEvent=function(){
    ...
    this.checked(this.jsrvalue.checked=!this.jsrvalue.checked);
    return false;
  };
  this.type="checkBox";
...
UI.crGroup={selectMemberIndex:-1,flow:false,initGroup:function(D){
    var A=this;
    ...
    var C=this,E={id:null,name:null,width:null,text:null,value:null,checked:null},D;
    for(var B in E){
      E[B]=A[B]||null;
    }
    E.name=C.name;
...
UI.CheckBoxGroup=function(A,C,D,B){
  ...
  this.dom=Jsr.extend(jsr(A),JTag);
  this.name=C;
  this.textWidth=D;
  this.memberClass=UI.CheckBox;
  this.srcMember=jsrB("input");
...
UI._checkBoxGroup={getValues:function(){
...
UI._radioGroup={getValue:function(){
    if(this.selectMemberIndex<0){
      return null;
    }
    return this.getMemberByIndex(this.selectMemberIndex).getValue();
  },selected:function(A){
    if(!this.members.length||this.members[0].jsrvalue.disabled){
      return false;
    }
    var B=this,C=null;
    if(typeof A=="string"){
...
UI.SliderBar=function(A){
  ...
  this.jsrvalue.type="hidden";
  this.jsrvalue.name=A.name||A.id;
  this.jsrvalue.value=A.minValue||0;
  this.dom=Jsr.extend(jsr(A.id),JTag);
  this.dom.id=Jsr.getTagID();
  this.setMinValue(A.minValue);
  this.setMaxValue(A.maxValue);
  this.render(A);
};
UI._sliderBar={render:function(E){
    ...
    B.jsrvalue.value=Math.ceil(parseInt(B.slideBlock.style.marginLeft.match(/\d*/))/(B.slideAxle.offsetWidth-10)*(B.max-B.min))+B.min;
    B=A=null;
  },autoSize:function(){
    var A=jsrP(this.dom);
    this.dom.setWidth(A.offsetWidth);
    this.slideAxle.setWidth(A.offsetWidth-4);
    this.setValue(this.jsrvalue.value);
    A=null;
  },disable:function(A){
    ...
    this.slideBlock&&this.setValue(this.jsrvalue.value);
...
UI.Button=function(A){
  ...
  this.dom=Jsr.extend(jsrB("div"),JTag);
  this.jsrvalue=Jsr.extend(jsr(A.id),JTag);
  this.jsrvalue.id=Jsr.getTagID();
  this.render(A);
};
UI._button={render:function(D){
    var G=this,B=G.dom,F=G.jsrvalue,E,C=false,H=false;
    (E=jsrP(F))&&E.insertBefore(B,F);
    ...
    this.jsrvalue.setHTML(A||"Button");
    B&&(this.dom.title=B);
  }};
UI.GroupBox=function(A){
  ...
  this.dom=jsr(A.id);
  this.dom.id=Jsr.getTagID();
  this.body=jsrN("div",this.dom)[0];
  this.render();
  this.setCaption(A.caption);
  this.expand(this.isExpand=A.expanded!=false);
};
UI._groupBox={render:function(){
    var B=this,A=B.dom.insertBefore(jsrB("legend"),B.body);
...
UI.container={initCtn:function(E){
    ...
    },H=this,G=(H.dom=Jsr.extend(jsr(E.id),JTag));
    ...
    G.id=Jsr.getTagID();
    G.status=0;
    var B=jsrA(jsrN("div",G)),I,A,C;
    if(E.close){
      F(C=jsrB("div"),["0 0","15px 0"],3);
      C.onmousedown=function(){
        H.hide();
      };
    }
    if(E.max){
      F(A=jsrB("div"),["0 30px","15px 30px"],2);
    ...
    if(E.min){
      F(I=jsrB("div"),["0 45px","15px 45px"],1);
    ...
    if(E.icon){
      ...
      G.bounds={l:0,t:0,w:0,h:0};
    }
    H.caption=B[3].lastChild.appendChild(jsrB("span"));
    H.captionBar=B[3];
    H.body=B[7];
    H.restore=D;
...
UI.winModel={_hide:function(){
    ...
    if(A.flash){
      if(A.fromObjct){
        ...
        D={l:B.tmpBounds.l+(B.tmpBounds.w-60)/2,t:B.tmpBounds.t+(B.tmpBounds.h-30)/2,w:60,h:30};
      }
      Jsr.boundsTo({fromObj:B.tmpBounds,toObj:D});
    }
    C&&C();
    A.isShow=false;
    A=B=C=D=null;
    ...
    if(E.flash){
      ...
      Jsr.boundsTo({fromObj:D,toObj:B.tmpBounds,callBack:function(){
          B.style.display="";
        }});
      E.fromObjct=C;
    }else {
    ...
    A.style.left=B.l=(document.body.clientWidth-B.w)/2 + 'px';
    A.style.top=B.t=(document.body.clientHeight-B.h)/2 + 'px';
    A=B=null;
  },hideIcon:function(A){
    ...
    var D=this,B=D.dom;
    D.clearPanel();
    D.body.appendChild(jsr(C)).style.display="";
...
UI.Dialog=function(A){
  ...
  this.initCtn({id:jsrB("div"),icon:true,close:true});
...
UI.alert=function(){
  ...
  B.flash=false;
  B.initCtn({id:jsrB("div"),icon:true,close:true});
  ...
  B.msg=jsrN("td",B.body)[0];
  A=new UI.Button({id:jsrB("button"),caption:"\u786e \u5b9a",width:70});
  A.bind(function(){
    B.hide();
  });
...
UI.confirm=function(){
  ...
  A.flash=false;
  A.initCtn({id:jsrB("div"),icon:true,close:true});
  ...
  A.msg=jsrN("td",A.body)[0];
  B=new UI.Button({id:jsrB("button"),caption:"\u786e \u5b9a",width:70});
  B.bind(function(){
    A.hide(true);
  });
  A.body.appendChild(B.dom);
  B=new UI.Button({id:jsrB("button"),caption:"\u53d6 \u6d88",width:70});
  B.bind(function(){
    A.hide(false);
  });
  A.body.appendChild(B.dom);
...
Jsr.drag=function(D,H){
  ...
  function E(E){
    ...
    F.cursor="move";
    F.display="";
    if(isFF || (navigator.userAgent.toLowerCase().indexOf("chrome") != -1)){
      window.captureEvents(Event.MOUSEMOVE|Event.MOUSEUP);
    }else {
      UI.dragLayer.setCapture(Event.MOUSEMOVE|Event.MOUSEUP);
    }
...
Jsr.boundsTo=function(D){
	//try{
  var E=function(){
	  $(B).attr("left", D.fromObj.l+Math.floor(A.l*C/10));
	  $(B).attr("top", D.fromObj.t+Math.floor(A.t*C/10));
	  $(B).attr("width", D.fromObj.w+Math.floor(A.w*C/10));
	  $(B).attr("height", D.fromObj.h+Math.floor(A.h*C/10));
    C++;
    if(C<11){
      E.defer(12);
    }else {
  ...
  },A={l:D.toObj.l-D.fromObj.l,t:D.toObj.t-D.fromObj.t,w:D.toObj.w-D.fromObj.w,h:D.toObj.h-D.fromObj.h},B=UI.dragLayer.style,C=0;
  $(B).attr("left", D.fromObj.l);
  $(B).attr("top", D.fromObj.t);
  $(B).attr("width", D.fromObj.w);
  $(B).attr("height", D.fromObj.h);
  $(B).attr("display", "");
  E();
	//}catch(err){alert(err.Description);}
};
onLoad.add(function(){
  UI.dragLayer=jsrB("div");
  UI.dragLayer.className="drag-layer";
  UI.dragLayer.style.display="none";
  UI.overLayer=jsrB("div");
...
var JTag={isJTag:true,add:function(A){
    ...
    return {l:A.x,t:A.y,w:this.offsetWidth,h:this.offsetHeight};
  },remove:function(){
    if(isFF){
      jsrP(this).removeChild(this);
    }else {
      this.removeNode(true);
    }
  }},VObject={disable:function(A){
    this.jsrvalue.disabled=A;
  },focus:function(){
    try{
      this.jsrvalue.focus();
    }
    ...
    this.jsrvalue.readOnly=A;
  },setValue:function(A){
    this.jsrvalue.value=A;
  },getValue:function(){
    return this.jsrvalue.value.trim();
  },reset:function(){
    this.jsrvalue.value=this.para.value||"";
  }};